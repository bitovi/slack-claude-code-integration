...
function sign(value) {
  return crypto.createHmac('sha256', SLACK_STATE_SECRET).update(value).digest('hex');
}
...
app.get('/oauth/callback', async (req, res) => {
  ...
  if (!code) {
    return res.status(400).send(
      errorPage('Something went wrong', 'Slack did not return an authorization code. Start again.')
    );
  }
  ...
  try {
    ...
    return res.status(502).send(
      errorPage('Could not reach Slack', 'The token exchange failed. Try again in a moment.')
    );
  }
  ...
  if (!data.ok) {
    ...
    return res.status(400).send(errorPage('Slack rejected the request', message));
  }

  const token = data.authed_user && data.authed_user.access_token;
  if (!token || !token.startsWith('xoxp-')) {
    console.error('No user token in response; app may be requesting bot scopes only.');
...
app.use((_req, res) => res.status(404).send(errorPage('Page not found', 'Start from the beginning.')));
